<template>
	<KQueryList>
		<KQueryTable>
			<KTableBar title="分类" :columns="columns" @refresh="fetchData">
				<template #buttons>
					<el-button
						v-access="'contentmanagement:classify:add'"
						type="primary"
						:icon="useRenderIcon('Plus')"
						@click="() => handleEdit({ title: '新增分类', effect: fetchData })"
					>
						新增
					</el-button>
				</template>
				<template #default="{ size, dynamicColumns }">
					...
					>
						<template #operation="{ row }">
							<el-button
								v-access="'contentmanagement:classify:edit'"
								class="!m-0"
								link
								type="primary"
								:size="size"
							...
							<el-button
								v-access="'contentmanagement:classify:del'"
								class="!m-0"
								link
								type="danger"
								:size="size"
...
import { deleteApi, pageApi } from "./utils/classify-api"
...
const {
	...
	hasPermission: () => access.hasAccess("contentmanagement:classify:detail"),
})
const columns = [
	{
		label: "",
