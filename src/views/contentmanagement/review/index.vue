<template>
	<KQueryList>
		<KQueryTable>
			<KTableBar title="评论" :columns="columns" @refresh="fetchData">
				<template #buttons>
					<el-button
						v-access="'contentmanagement:review:add'"
						type="primary"
						:icon="useRenderIcon('Plus')"
						@click="() => handleEdit({ title: '新增评论', effect: fetchData })"
					>
						新增
					</el-button>
				</template>
				<template #default="{ size, dynamicColumns }">
					...
					>
						<template #operation="{ row }">
							<el-button
								v-access="'contentmanagement:review:edit'"
								class="!m-0"
								link
								type="primary"
								:size="size"
							...
							<el-button
								v-access="'contentmanagement:review:del'"
								class="!m-0"
								link
								type="danger"
								:size="size"
...
import { deleteApi, pageApi } from "./utils/review-api"
...
const {
	...
	hasPermission: () => access.hasAccess("contentmanagement:review:detail"),
})
const columns = [
	{
		label: "",
