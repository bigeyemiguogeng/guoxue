<template>
	<KQueryList>
		<KQueryTable>
			<KTableBar title="收藏" :columns="columns" @refresh="fetchData">
				<template #buttons>
					<el-button
						v-access="'usermanagement:collection:add'"
						type="primary"
						:icon="useRenderIcon('Plus')"
						@click="() => handleEdit({ title: '新增收藏', effect: fetchData })"
					>
						新增
					</el-button>
				</template>
				<template #default="{ size, dynamicColumns }">
					...
					>
						<template #operation="{ row }">
							<el-button
								v-access="'usermanagement:collection:edit'"
								class="!m-0"
								link
								type="primary"
								:size="size"
							...
							<el-button
								v-access="'usermanagement:collection:del'"
								class="!m-0"
								link
								type="danger"
								:size="size"
...
import { deleteApi, pageApi } from "./utils/collection-api"
...
const {
	...
	hasPermission: () => access.hasAccess("usermanagement:collection:detail"),
})
const columns = [
	{
		label: "",
